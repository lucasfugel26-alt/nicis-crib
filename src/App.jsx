...
import { ChartNoAxesCombined, Heart, Home, ListPlus, Plus, WalletCards } from "lucide-react";
import { AppShell } from "./components/AppShell.jsx";
import { loadState, put, rememberCategory, rememberShop, remove } from "./data/db.js";
import { normalizeName, suggestCategory, todayISO } from "./data/categories.js";
  ...
  const actions = useMemo(() => {
    ...
    return {
      ...
      saveCategory: async (category) => {
        ...
        const id = category.id || `custom-${normalizeName(category.name) || Date.now()}`;
        const nextCategory = { id, name: category.name.trim(), color: category.color };
        const baseCategoryIds = ["kueche", "bad", "moebel", "deko", "reinigung", "technik", "lebensmittel", "sonstiges"];
        const nextSettings = { ...state.settings };

        if (baseCategoryIds.includes(id)) {
          nextSettings.categoryOverrides = {
            ...(nextSettings.categoryOverrides || {}),
            [id]: { name: nextCategory.name, color: nextCategory.color }
          };
        } else {
          const currentCustom = nextSettings.customCategories || [];
          nextSettings.customCategories = isNew
            ? [...currentCustom, nextCategory]
            : currentCustom.map((item) => (item.id === id ? nextCategory : item));
        }

        await put("settings", nextSettings);
      ...
      deleteExpense: async (id) => {
        ...
        if (expense?.purchaseId && state.expenses.filter((item) => item.purchaseId === expense.purchaseId).length <= 1) {
          await remove("purchases", expense.purchaseId);
        }
        setState((current) => ({
          ...current,
          expenses: current.expenses.filter((item) => item.id !== id),
          purchases: expense?.purchaseId
            ? current.purchases.filter((item) => item.id !== expense.purchaseId)
            : current.purchases
        }));
        setToast("Einkauf gelöscht.");
      }
    };
  }, [state]);
