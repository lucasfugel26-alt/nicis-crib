...
import { ChevronDown, Crown, Euro, PieChart, ReceiptText, ShoppingBag, Trash2 } from "lucide-react";
import { getCategories, getCategoryMap, money } from "../data/categories.js";
...
export default function Dashboard({ state, actions, go }) {
  ...
  const categoryById = getCategoryMap(state.settings);
            ...
            {byCategory.filter((item) => item.spent > 0).map((item) => (
              <div key={item.id}>
                <div className="mb-2 flex items-center justify-between text-sm font-bold">
                  <CategoryPill id={item.id} settings={state.settings} />
            ...
            {state.expenses.slice(0, 6).map((item) => (
              <div key={item.id} className="flex items-center justify-between gap-3 rounded-3xl bg-white/54 p-3">
                ...
                <div className="min-w-fit text-right">
                  ...
                  <CategoryPill id={item.category} settings={state.settings} />
                </div>
              </div>
            ))}
          </div>
        ...
        {byCategory.map((item) => {
          ...
          return (
            <article key={item.id} className={`rounded-[2rem] bg-white/55 p-5 text-left shadow-soft transition ${isOpen ? "md:col-span-2 xl:col-span-2" : ""}`}>
              ...
              >
                <div>
                  <item.icon className="mb-4" size={24} style={{ color: categoryById[item.id]?.color || item.color }} />
